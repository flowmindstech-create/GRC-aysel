        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
          ...
          <form onSubmit={async e => { e.preventDefault(); if (!body.trim()) return; setLoading(true); await onSave(subject.trim(), body.trim()); setLoading(false) }} className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <p className="text-[10px] mt-1 text-emerald-400 flex items-center gap-1"><Lock className="w-3 h-3" /> Encrypted — only those with the code can view</p>
            </div>
    ...
    <div className="space-y-5">
      ...
      <div className="card p-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex-1 min-w-52">
          ...
          <p className="text-[11px]" style={{ color: 'var(--muted-fg)' }}>The report content is encrypted — an officer code is required to view it (even an admin cannot view it without the code).</p>
        </div>
        {code ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleUnlock} className="px-3 py-2 rounded-lg text-xs font-semibold text-white" style={{ background: 'var(--brand-500)' }}>Unlock</button>
          </div>
        )}
        {code && (
            ...
            <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="group hover:bg-black/[0.02] dark:hover:bg-white/[0.02] align-top" style={{ borderBottom: '1px solid var(--border)' }}>
              ...
              <td className="px-3 py-3.5 max-w-xs">
                {revealed[r.id] ? (
                  ...
                  <button onClick={() => reveal(r)} className="inline-flex items-center gap-1 text-xs font-semibold" style={{ color: 'var(--brand-500)' }}><Eye className="w-3 h-3" /> Unlock</button>
                ) : (
                  <span className="inline-flex items-center gap-1 text-xs" style={{ color: 'var(--muted-fg)' }}><Lock className="w-3 h-3" /> Encrypted</span>
                )}
              </td>
