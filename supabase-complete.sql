...
CREATE TABLE IF NOT EXISTS risks (
  ...
  category                TEXT NOT NULL CHECK (category IN ('cybersecurity','financial','operational','legal_compliance','strategic')),
  level                   TEXT NOT NULL CHECK (level IN ('minimal','low','medium','high','critical')),
  status                  TEXT NOT NULL DEFAULT 'open' CHECK (status IN ('open','backlog','in_progress','review','done','solved')),
...
(
  ...
  'legal_compliance', 'high', 'in_progress', 4, 4, 'Implement double opt-in flow, audit existing contact list, update privacy policy.',
...
(
  ...
  'operational', 'low', 'solved', 2, 3, 'Purchase and placement of standard fully equipped first aid boxes for each floor.',
